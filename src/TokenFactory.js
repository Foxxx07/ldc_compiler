...
export default class TokenFactory{
	...
	static create(value, pos){
		...
		switch(char){
			...
			case '[':
				return new Token('bracket-start', '[', pos);
			case ']':
				return new Token('bracket-end', ']', pos);
			case '{':
				return new Token('curly-bracket-start', '{', pos);
			case '}':
				return new Token('curly-bracket-end', '}', pos);
			case '|':
				return new Token('pipe', '|', pos);
			case '<':
				return new Token('angle-bracket-start', '<', pos);
			case '>':
				return new Token('angle-bracket-end', '>', pos);
			case '"':
				sub_current = 1
